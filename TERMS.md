...
| `Action` | added, amended, enacted, repealed, repealed and added, repealed conditionally, renumbered, adopted | The history verb. In front of `Stats.` the target is the action, then that year and chapter. `renumbered from Sec. 8` keeps the earlier section on `Credit.source`. `adopted 1879` is the action and a year, with no measure. |
| `Credit` | by | `by` joins the section credit to the measures. A `Stats.` line is one `Session`. A constitution line is one `Credit`: the action and the section on the left, the measures on the right. |
| `Measure` | chapter | `Measure.RESOLUTION` is `Res.Ch. 90, 1974`. That year is the legislative year of the resolution. `Session.chapter` is a bill chapter of the Statutes. |
| `Measure` | initiative | `Initiative measure` is `Measure.INITIATIVE`. The electors proposed it, so the credit has no resolution chapter. |
| `Credit` | effective | The date before `by`, such as `Nov. 5, 1974`, is the election day on `Credit.when`. `Occasion.EFFECTIVE` is the word `Effective` and its own calendar day. |
| `Occasion` | effective, operative, inoperative, approved, applicable, superseded, repealed | A separate token beside a calendar day. Joiners such as `on` and `as of` may sit between them. The occasion target is that day's `YYYY-MM-DD`. The date token stays the printed day. |
| `Note.OVERRIDE` | `notwithstanding` | The paired object is the target: `any other provision`, `any other law`, `section`, or `unit`. |
| `Note.EXCEPTION` | `except as provided`, `unless otherwise` | The paired object is `in section`, `in subdivision`, `provided`, or `qualified`. |
| `Note.LIMIT` | `subject to` | The paired object is `section`, `approval`, `jurisdiction`, `unit`, or `the following`. |
| `Note.PROVISO` | `provided that` | The phrase introduces the qualifying clause. It has no stock noun, so the target stays `proviso`. |
...
| `Credit` | one parenthetical history. Fields are `action`, `subject`, `source`, `when`, and `measures`. `Sec. 20 added Nov. 5, 1974, by Prop. 7. Res.Ch. 90, 1974.` The date is the election day | `CreditMixin.credits` |
| `Measure.PROPOSITION` | `Prop. 7`, `Proposition 7`, `Prop. 1-a`, `Prop. 1A`. The target is `prop` and the ballot number, letter included | `CreditMixin.credits` |
| `Measure.RESOLUTION` | `Res.Ch. 90, 1974` and `Resolution Chapter 90, 1974`. The chapter, then the legislative year. An extra session stays on the measure: `1st Ex. Sess.` or `2003-04 5th Ex. Sess.` | `CreditMixin.credits` |
| `Measure.INITIATIVE` | `Initiative measure`. No chapter number | `CreditMixin.credits` |
| `Measure.AMENDMENT` | `A.C.A. 14`, the older name of a legislative constitutional amendment | `CreditMixin.credits` |
| `Measure.SESSION` | `Stats.` year, `Ch.` chapter, `Sec.` of the enrolled bill. The same record as `Session` | `CreditMixin.credits`, `find_links` |
| `ArticleMark` | a constitution article printed as a roman numeral and an optional letter. `'XIX'`, `'XIII A'`, and `Article XIX C`. The target is `CONS` and that label. Quotes mark a bare numeral | `RomanMixin.articles` |
...
| `Note.DEFINITION` | a quoted term, or several joined by `or` or `and`, followed by `means`, `includes`, `does not include`, `refers to`, `may include`, or `shall`. The target is that term | `annotate` |
| `Note.COMPOUND` | a hyphenated word, such as `out-of-pocket`. The target is that word | `annotate` |
| `PublicLaw` | a book on the public-law shelf. `book` is the hyphenated `101-336`. The target is `PL`, the Congress, then the law number. The mark is `Cite.BOOK`, not a section range | `ShelfMixin.public_laws` |
| `FederalCode` | a section of a United States Code title. `book` is the title. The target is `USC`, the title, then the section. The mark is `Cite.BOOK` | `ShelfMixin.federal_codes` |
| `StatutesAtLarge` | a page of a Statutes at Large volume. `book` is the volume. The target is `STAT`, the volume, then the page. The mark is `Cite.BOOK` | `ShelfMixin.statutes` |
| `Cite.SECTION`, `Cite.RANGE`, `Cite.SERIES`, `Cite.BOOK` | how many sections the sign introduces. `book` is a hyphenated shelf identity, not a range | `find_citations`, `ShelfMixin` |
...
| A heading span, or the URL tree | `query.outline`, `query.law_tree`. `GET /tree/{url}` adds `crumbs` and `contents`, the children of that node as links |
| Every surface on one text | `marks.layers(text, code=...)`. One `Span` per reading: `Layer.NOTE` from `annotate`, `CANON` from `find_signals`, `CLAUSE` from `find_clauses`, `MENTION` and `RELATION` from `mentions`, `ABBREVIATION` from `abbreviations`, and `NEEDLE` from the word classes `consider` returns for that book. `only` keeps one layer. `counts` tallies them. A bare section number takes the book the sentence named, read from `find_links`, so `Section 7280 of the Revenue and Taxation Code` is `RTC 7280`. It does not open the index |
| A session law | `query.session_law`, `query.act` |
| A constitution credit | `CreditMixin.credits`. One `Credit`: action, subject, source, election day, and the measures after `by` |
...
| The React reader | `GET /reader` and `/reader/{view}`. The client mounts on `#root` there; the same place as a script-free document stays inside `noscript`. A view is `tree`, `section`, `search`, `cite`, `outline`, `diagram`, `graph`, `closure`, or `annotations`, and each one is a URL, so every place is linkable. `/view` is that same place without the script, and a link the JSON carries is the `/view` one |
| The closed sets | `GET /surfaces`. One list per enum: `layer`, `note`, `canon`, `clause`, `cite`, `join`, `cut`, `kind`, `relation`, `quantity`, `gap`, `use`, `instrument`, and `hint`, plus the cut `units` in order. The client builds its legend and its filters from this reply and keeps no copy of the words |
| Every reading on a section | `GET /marks/{code}/{number}`. `spans` is keyed by the same cut `path` the section's `nodes` carry, and each row is a `layer`, a `kind`, `start` and `end` into that cut's words, the `text`, the `target`, the `reading` the record carries, and an `href` when the span opens something. `counts` is how many of each kind the section holds. `layer=note,clause` narrows it; an unknown layer is a miss. Readings overlap, so a phrase is several spans and not one slice |
| The sections that name this one | `GET /citing?citation=CIV+1940`. A stored citation edge, in both directions: `names` is true when that section names this one. `Indexer.reference_edges` is the list, and only an exact end is kept, so `CIV 1940` does not collect `CIV 1940.1` |
| A closure the client opened | `GET /closure`. `code` is the book. `division`, `title`, `part`, `chapter`, `article`, `section`, and `subdivision` remember the unit above. `session` is the year. `hops` is the walk, and `all` follows until a section repeats. `only` is a comma-separated book list. `same=1` stays in the open book. `q` searches inside the place. `use` is `read`, `find`, `refs`, or `gaps`. A missing use follows the filters that were sent |
| The walk a closure drew | `use=refs`. `nodes` is one row per section the walk opened, with the `hop` it was reached on and an `href`. `edges` is `source`, `target`, and `label` for every hop, not only the first. `links` is every pointer in the words. `chart` is the same walk as the mermaid export |
| The citations a walk missed | `use=gaps`. `structure.review` over the same hops. Each row is the `gap`, the `phrase`, the `code`, the `section`, and an `href`. A gap is a reading to check. It is not a defect in the text |
...
| One section in a year | `GET /section/{code}/{number}?session=`. Empty uses the newest year. The reply still includes `pieces`, `previous`, and `next`. It also carries `units` (the ladder with the number the index stored), `crumbs` (that ladder as links, each rung named by the caption whose own words name it), `contents` (the ordered children of the rung above, with `current` on this one and `short` for the bare number), `nodes` (the section as its cuts, each with its own pieces), `credit` (the history line as pieces), `links` (each statute the words name, once), and `formats` (the same section under each `Hint`) |
| The book list | `GET /codes`. Each row is `code` and `title`, in the Legislature's order |
| Publication years | `GET /sessions`. The years stored on `SESSION` |
| A heading outline | `GET /outline`. `code`, `start`, and `end` bound the span. `session` selects the year. The reply is `nodes`. The section text stays out |
| One expression | `GET /cite`. `q` is a section, a span, a named act, or a session credit. `session` selects the year. The reply is that helper's record |
| A stored graph | `GET /diagram/codes`, `/diagram/vesting`, or `/diagram/enactments`. `code` narrows vesting and enactments. `chart` is the flowchart. `edges` is the same graph as `source`, `target`, `label`, `weight`, and the href each end opens. `Indexer.code_edges`, `vesting_edges`, `enactment_edges`, and `reference_edges` are that list; each chart is drawn from it |
...
- React is the client, mounted at `/reader` on `#root`, built from `client/src` into `static/reader.js` and `static/reader.css` by `npm run build`. It reads the JSON routes and keeps no copy of the words: the legend and the filters come from `/surfaces`. The library is `/tree`, one level at a time. The reader is `/section` and `/marks`: the crumbs, the cuts, every reading drawn as a layer over the words, the history credit, the sections beside it, the sections that name it, and the walk. Markup is layered: a note takes a tint, a clause a bar, a canon a dotted rule, a name an underline, a word class a ring, so a phrase that is three readings at once shows all three. A span that names a place opens it on a click; hovering any span opens the card, and so does a double-click on any word, which is why a word is not a component until it is asked for. The card carries the reading the record already holds, the heading `this chapter` resolved, and the ways out: the section the citation names, the same words in search, and the stored rows for that note. The DAG is the edge list from a closure's `use=refs` and from `/diagram` (source, target, label), not the mermaid string; a node opens the next section. Mermaid `flowchart TD` remains the static export. Do not use a timeline, a mindmap, a git graph, or a gantt chart. A walk of a few sections is drawn as the walk; a whole book-to-book relation is hundreds of edges and is drawn as a grid, row cites column, shaded by how many citations that pair holds. Stored graphs are codes, vesting, and enactments. The Jinja document stays available when scripts do not run, and `/reader` keeps it inside `noscript`.
- The client's own words: `place.js` is every URL and the reason a miss gives; `api.js` is the routes; `marks.jsx` is the marks, the cuts, and the card; `graph.jsx` is the drawing; `layers.jsx` is the layered drawing and the reading control; `reading.js` is what the open page shares; `library.jsx`, `section.jsx`, `find.jsx`, and `closure.jsx` are the views; `main.jsx` is the shell and the keys. A link the server sends is a `/view` path, and `local()` is the one place that turns it into the same place in the reader.
