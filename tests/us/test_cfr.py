...
def test_sections_from_synthetic_fixture(tmp_path):
    ...
    assert all(r['LAW_CODE'] == '44' for r in rows)
    ...
    try:
        ...
        assert len(stored) == 2
        assert stored[0][0] == '44:1.1'
        assert stored[0][1] == '44'
        assert stored[0][2] == '1.1'
        assert 'informal rulemaking' in stored[0][3]
        assert stored[0][4] == '44 CFR 1.1'
        assert stored[1][0] == '44:1.2'
        assert stored[1][4] == '44 CFR 1.2'
    finally:
        db.close()
